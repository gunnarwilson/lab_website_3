...
app.get('/team_stats', function(request, res) {
  var query1 = 'SELECT * FROM football_games;';
  var query2 = 'SELECT COUNT(*) AS games_won FROM football_games WHERE home_score > visitor_score;';
  var query3 = 'SELECT COUNT(*) AS games_lost FROM football_games WHERE visitor_score > home_score;';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
  ...
  .then(data => {
  	res.render('pages/team_stats',{
  			my_title: "WHERE DOES THIS TITLE SHOW UP",
  			result_1: data[0],
  			result_2: data[1],
  			result_3: data[2]
  		})
